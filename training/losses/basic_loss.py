...
try:  # Prefer BasicSR's extractor if available
    from basicsr.archs.vgg_arch import VGGFeatureExtractor  # type: ignore
    ...
    class VGGFeatureExtractor(nn.Module):
        ...
        def __init__(
            ...
            required_indices = [idx for idx, name in name_by_index.items() if name in layer_name_list]
            if not required_indices:
                raise ValueError(f"No matching VGG layers found for names: {layer_name_list}")
            ...
            self._index_to_name = {idx: name for idx, name in name_by_index.items() if name in layer_name_list}

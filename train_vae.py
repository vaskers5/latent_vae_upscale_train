#!/usr/bin/env python3
"""Single-node VAE training entry point."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a VAE using YAML configuration files")
    parser.add_argument(
        "-c",
        "--config",
        action="append",
        dest="configs",
        help="Path to a YAML config file. Can be supplied multiple times to merge overrides.",
    )
    parser.add_argument(
        "--exp-name",
        dest="exp_name",
        help="Optional experiment name used for run directory naming and logging.",
    )
    parser.add_argument(
        "-o",
        "--override",
        ...
        default=[],
        metavar="KEY=VALUE",
        help="Manual override (supports dotted keys, e.g. model.dtype=bfloat16).",
    )
...
def main() -> None:
    args = parse_args()
    config_paths = args.configs or [str(DEFAULT_CONFIG)]

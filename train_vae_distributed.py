...
# ну работа, потом диплом, потом Коля, еще геймпады эти дурацкие
# --------------------------- VAE ---------------------------
def get_core_model(model):
    # Сначала разворачиваем модель от Accelerate
    if hasattr(accelerator, "unwrap_model"):
        model = accelerator.unwrap_model(model)
    
    # Затем, если модель была скомпилирована, получаем оригинальную модель
    # (torch.compile оборачивает модель в дополнительный объект)
...
def generate_and_save_samples(step=None):
    try:
        temp_vae = get_core_model(vae).eval()
        with torch.no_grad():
